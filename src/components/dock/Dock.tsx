...
function DockItem({ children, className = '', onClick, active, mouseX, spring, distance, magnification, baseItemSize }: DockItemProps) {
  ...
  const targetSize = useTransform(mouseDistance, [-distance, 0, distance], [baseItemSize, magnification, baseItemSize], { clamp: true });
  ...
  return (
    <motion.div
      ...
      aria-haspopup="true"
    >
      {Children.map(children, child => cloneElement(child, { isHovered } as Record<string, unknown>))}
    </motion.div>
  );
}
